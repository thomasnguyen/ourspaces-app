// POC: turn a public-domain painting (JPEG) into paint-by-number vector data.
...
//
//   node scripts/trace-artwork.mjs .context/art-candidates/starry-night.jpg out-prefix [colors] [scale]
//
// Emits <out-prefix>-finished.svg (full color) and <out-prefix>-board.svg
// (muted underlay + numbered blank regions), plus stats on stdout.
...
const [, , input, outPrefix, colorsArg, scaleArg] = process.argv;
if (!input || !outPrefix) {
  console.error("usage: trace-artwork.mjs <input.jpg> <out-prefix> [colors=7] [targetWidth=560]");
  process.exit(2);
}
